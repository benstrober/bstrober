...
```{r}
maryland_data <- trept_ncaa |> filter(Team == "Maryland")
...
```{r}
ggplot(team_correlations, aes(x = correlation_3P_wins, y = correlation_3PM_wins)) +
...
There's a lot to unpack from this. Where does Maryland stand among all these teams?

```{r}
ggplot(team_correlations, aes(x = correlation_3P_wins, y = correlation_3PM_wins)) +
  geom_point(color = "darkblue") +
  geom_smooth(method = "lm", color = "red", linetype = "dashed") +
  geom_point(data = team_correlations[team_correlations$Team == "Maryland", ],
             aes(x = correlation_3PM_wins, y = correlation_3PM_wins),
             color = "red", size = 3) +
...
If Maryland was more consistent from three, it may have seen more success in some of its recent tournament runs. By being just average on offense and from three, the team has used a defensive idenity recently to scrape together wins. The room for improvement is evident.


```{r}
ggplot() +
  geom_dumbbell(
    data = maryland_vs_ncaa,
    aes(y = Season, x = avg_ncaa_3PM, xend = avg_3pm),
    colour = "grey",
    colour_x = "blue",
    colour_xend = "red"
  ) +
  labs(
    title = "Below Average From 3 Is a Below Average Team"
  )
```
...
Maryland's only sub .500 season came in 2023.  Maryland has typically won a fair amount of games. Despite being below average shooting threes, Maryland has found ways to overcome it to an extent. However, it has never really obtained elite status outside of 2019 when there wasn't a tournament.

Maryland has been pretty consistent with the amount of threes it has taken over the years. In years where they were better teams overall, such as 2019, they typically took more threes. Maryland has been pretty consistently between 6-8 threes made per game. Although the team has done well in a good amount of years, them falling behind NCAA averages reveals lack of offensive output holds them back from being more dynamic.

NCAA obviously has so many teams to pick through, and many of them have different strengths. When all of them are clustered, where does Maryland end up?
...
```{r}
three_point_contribution <- winlosslogs |>
...
#  geom_point(aes(group = Team), alpha = 0.5) +
  ...
  labs(
    title = "Maryland Needs to Improve Offensively Overall",
    subtitle ="Despite Maryland's decent win percentage over the years, offensive struggles are evident",
    x = "Total Points",
  ...
  theme_minimal()
```
Maryland is slightly below average compared to NCAA in scoring. As for percentage of points coming from three, Maryland is about average and this reveals Maryland isn't necessarily reliant on three-point shooting. However, if they are really awful from three, they p will suffer. 2023 is a good example because in Maryland's worst 3 pt shooting year, the team had its only year with a losing record.

Three point shooting isn't everything for teams in NCAA. Kentucky has a winning percentage of over 75 despite using the three pointer for under 25% of its points. Even without making a lot of threes, plenty of teams have found ways to be exciting and score in other ways to win. Offense comes in many forms.A great offense could push Maryland over the hump its struggled to surpass in recent years.

Only four teams in NCAA are over 35% for percentage of points coming from threes. Overall, Maryland has been pretty average as an offensive basketball team, but the win total has changed the perception of the team. Even in great years for Maryland, the three-point shooting has never been other wordy compared to other NCAA teams. If the team finds other ways to win, people won't notice as much.

With that being said, Maryland could benefit from improvements on offense. It's clear the team can win using its defensive prowess, but the lack of firepower on the offensive end is what separates it from being a great team as opposed to a good team.
