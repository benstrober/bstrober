...
TJ NeSmith is the first player in the data that comes up. His true shooting PCT was 75%.
...
```{r}
attendance |> filter(Conference %in% powerfive) |> select(Institution, Conference, `2024`) |> arrange(desc(`2022`)) |> top_n(10)
```
...
filter(date %in% c("2023-08-17", "2023-08-20", "2023-08-24", "2023-08-27", "2023-08-31")) |>
   group_by(team) |>
  summarise(
    total_attempts = sum(sh_att),
    total_on_goal = sum(so_g),
...
The first five games of the season saw a greater percentage in shots on goal through five games. In 2023 the total was 53.57%, compared to 47% this year. However, through five games, the team has registered 9 more shots than last year.
